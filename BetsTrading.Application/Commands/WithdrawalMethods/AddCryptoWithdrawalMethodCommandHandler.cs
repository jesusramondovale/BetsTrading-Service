...
public class AddCryptoWithdrawalMethodCommandHandler : IRequestHandler<AddCryptoWithdrawalMethodCommand, AddWithdrawalMethodResult>
{
    private readonly IUnitOfWork _unitOfWork;

    public AddCryptoWithdrawalMethodCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<AddWithdrawalMethodResult> Handle(AddCryptoWithdrawalMethodCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var existing = await _unitOfWork.WithdrawalMethods.GetByUserIdTypeAndLabelAsync(
                request.UserId, "crypto", request.Label, cancellationToken);

            if (existing == null)
            {
                var entity = new WithdrawalMethod(
                    Guid.NewGuid(), request.UserId, "crypto", request.Label, jsonDoc,
