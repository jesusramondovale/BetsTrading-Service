...
namespace BetsTrading_Service.Services 
{
  public class UpdaterService(AppDbContext dbContext, ICustomLogger customLogger, FirebaseNotificationService firebaseNotificationService)
  {
    ...
    public async Task UpdateAssetsAsync(IServiceScopeFactory scopeFactory, bool marketHours, CancellationToken ct = default)
    {
      ...
      var query = !marketHours ?
        _dbContext.FinancialAssets.AsNoTracking().Where(fa => (fa.Group.Equals("cryptos", StringComparison.OrdinalIgnoreCase) 
                                                           || (fa.Group.Equals("forex", StringComparison.OrdinalIgnoreCase)))) 
        :  
     
        _dbContext.FinancialAssets.AsNoTracking().Where(a => !(a.Group.Equals("cryptos", StringComparison.OrdinalIgnoreCase) 
                                                          && !(a.Group.Equals("forex", StringComparison.OrdinalIgnoreCase)
                                                          && !(a.Group.Equals("commodities", StringComparison.OrdinalIgnoreCase)))));
    ...
    public async Task CreateBets(bool marketHoursMode)
    {
      ...
      try
      {
        var query = !marketHoursMode ?
               ...
               _dbContext.FinancialAssets.AsNoTracking().Where(fa => !(fa.Group.Equals("cryptos", StringComparison.OrdinalIgnoreCase)
                                                          && !(fa.Group.Equals("forex", StringComparison.OrdinalIgnoreCase)
                                                          && !(fa.Group.Equals("commodities", StringComparison.OrdinalIgnoreCase)))));
      ...
      try
      {
        var query = !marketHours ? 
          ...
          .Where(bz => _dbContext.FinancialAssets
              .Where(a => a.Group.Equals("cryptos", StringComparison.CurrentCultureIgnoreCase) || a.Group.Equals("forex", StringComparison.CurrentCultureIgnoreCase))
              .Select(a => a.Ticker)
              .Contains(bz.ticker))
      :   _dbContext.BetZones
          .Where(bz => _dbContext.FinancialAssets
              .Where(a => !a.Group.Equals("cryptos", StringComparison.CurrentCultureIgnoreCase) && !a.Group.Equals("forex", StringComparison.CurrentCultureIgnoreCase) && !a.Group.Equals("commodities", StringComparison.CurrentCultureIgnoreCase))
              .Select(a => a.Ticker)
              .Contains(bz.ticker));
  ...
  public class UpdaterHostedService(IServiceProvider serviceProvider, ICustomLogger customLogger) : IHostedService, IDisposable
  {
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
      ...
      {
        _ = Task.Run(() => ExecuteUpdateAssets(false));
      ...
      _marketHourAssetsTimer = new Timer(async _ =>
      {
        ...
        if (newYorkTime.DayOfWeek is >= DayOfWeek.Monday and <= DayOfWeek.Friday)
        {
          ...
          {
            await ExecuteUpdateAssets(true);
          }
        }
